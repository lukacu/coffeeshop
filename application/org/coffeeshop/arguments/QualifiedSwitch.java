 ...
 * optional value(s).
 * Values are retrieved from the ArgumentResult in the same manner they would
 * be retrieved for an equivalent FlaggedOption.
 * <p>
 * Additionally, the QualifiedSwitch's presence on the command line can be
 * determined via ArgumentResult.getBoolean(id).  This presents a small challenge
 * in the unlikely event that you're also using a BooleanStringParser with
 * the QualifiedSwitch; if you are, ArgumentResult.getBoolean(id) will <b>not</b>
 ...
 * ArgumentResult.getBooleanArray(id).  The first boolean in the array will be the
